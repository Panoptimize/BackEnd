...
public interface AgentPerformanceRepository extends JpaRepository<AgentPerformance, Integer> {
    ...
    @Query("SELECT AVG(ap.avgAbandonTime) as avg_abandon_time, AVG(ap.avgAfterContactWorkTime) as avg_after_contact_work_time, AVG(ap.avgHandleTime) as avg_handle_time, AVG(ap.avgHoldTime) as avg_hold_time FROM AgentPerformance ap WHERE DATE(ap.createdAt) = :date AND ap.agent.id =:agentId")
    AgentPerformance findAgentPerformanceByAgentIdDate(Instant searchDate, Integer agentId);
}
